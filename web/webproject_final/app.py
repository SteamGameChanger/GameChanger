...
def find():
    ...
    if len(final_key_XL) == 0 and len(final_key_Bert) == 0:
        ...
        for k in final_key_XL:
            if n < 5:
                ...
                XL_result.append(inner)
                n, inner = 0,[]
        XL_result.append(inner)
                    
        Bert_result, inner,n = [], [],0
        for k in final_key_Bert:
            if n < 5:
                inner.append(k)
                n += 1
            else:
                Bert_result.append(inner)
                n, inner = 0, []
        Bert_result.append(inner)
    ...
    for idx in top_indices:
        tmp = [df.iloc[idx]['Name'], str(df.iloc[idx]['AppID']), "https://store.steampowered.com/app/"]
        similar_games.append(tmp)

    # price recommend --------------------------------------------
    vectorizer_path = os.path.join(app.static_folder, 'pickle/tfidf_vectorizer.pkl')
    model_path = os.path.join(app.static_folder, 'pickle/xgb_model.pkl')
